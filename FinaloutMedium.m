...
for i=1:length(Data)
    ...
    %Checks to see if complete concentration data was provided
    if isnan(Data(i,9))
        continue
    end 
%checks to be sure proper chemistry is used
    ...
    if Data(i,12)==1
        ConcentrationData(count,1)=i;%Preserves the index in Data file for future reference
...
%Atleast one case where 0 was reported for the error in Concentration
...
for i=1:length(Data(ConcentrationData(:,1),7))
    ...
    %checking if 50 is in the interval of toxicity as EC54+-x is reported
...
for i=1:length(EP50data)
    for j=1:length(coatingregex)
        if ~isempty(regexp(CellDat{Data(EP50data(i,1),13),5}, coatingregex(j), 'once'))
            EP50data(i,8)=j;%Type of coating
        end
    end
end

Shapeaccount1=EP50data;
Shapeaccount2=EP50data;%originally applied the shape characteristics from
%the incomplete case analysis but wasn't finding a clear enough difference
%to keep including 
...
csvwrite('RData\OTMCompShape.csv',Shapeaccount1)
csvwrite('RData\OTMComp.csv',Shapeaccount2)
...
for i=1:length(Data)
    ...
    if isnan(Data(i,3)) || Data(i,4)==-9999 ||Data(i,5)==9999%Data(i,3) is size storage if NaN then no size data 
        continue
    end
    if isnan(Data(i,9))
        continue
...
csvwrite('RData\OTMIncomp.csv',Shapeaccount3)
